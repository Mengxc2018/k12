...
//                        deptOptional.get(),
    ...
    public void empCommit(Actor actor, EmpCommitForm form) {
        Integer dueyId = form.getDutyId();
        Duty duty = new Duty();
        if (dueyId != 0 || dueyId != null){
            duty = this.dutyRepository.findOne(Long.parseLong(dueyId.toString()));
        }
        Employee employee = this.getRepository().findByActorId(form.getActorId());
        if (employee == null){
            employee = new Employee();
        }
        employee.setFileId(0);
        employee.setSalary(0f);
        employee.setActorId(form.getActorId());
        employee.setName(form.getName());
        employee.setDeptId(form.getDeptId());
        employee.setIdCard(form.getIdCard());
        employee.setSex(form.getSex());
        employee.setMobile(form.getMobile());
        employee.setEmergencyContact(form.getEmergencyContact());
        employee.setEmergencyContactMobile(form.getEmergencyContactMobile());
        employee.setNativePlace(form.getNativePlace());
        employee.setNation(form.getNation());
        employee.setPoliticsStatus(form.getPoliticsStatus());
        employee.setIsMarry(form.getIsMarry());
        employee.setHjAddress(form.getHjAddress());
        employee.setAddress(form.getAddress());
        employee.setEducation(form.getEducation());
        employee.setGraduateSchool(form.getGraduateSchool());
        employee.setSpecialty(form.getSpecialty());
        employee.setCgfns(form.getCgfns());
        employee.setRemark(form.getRemark());
        employee.setGraduate(form.getIsGraduate());
        employee.setHasDiploma(form.getIsHasDiploma());
        employee.setDuty(duty);
        employee.setParentActorId(form.getParentActorId());
        employee.setShow(true);
        employee.setSchoolId(actor.getSchoolId());
    ...
    public Collection<EmployeeDTO> findEmpCommit(Actor actor) {
        return employeeMapper.toDTOs(this.getRepository().findBySchoolIdAndSalaryNotNullAndSalary(actor.getSchoolId(), 0f));
    }
    ...
    public void managerUpdateEmp(Actor actor, ManagerUpdateForm form) {
        ...
        Employee employee = form.getId() == null
                ? this.getRepository().findOne(Long.parseLong(String.valueOf(id)))
                : this.getRepository().findByActorId(form.getActorId());
        ...
        employee.setLeave(false);
        employee.setLeaveAt(null);
        employee.setHasSocial(form.getIsHasSocial());

        employee.setOvertime(form.getOvertime() == null ? "0" : form.getOvertime());
        employee.setRest(form.getRest() == null ? "0" : form.getRest());
        employee.setAnnual(form.getAnnual() == null ? "0" : form.getAnnual());
        employee.setSick(form.getSick() == null ? school.getSick() : form.getSick());
        employee.setBarth(form.getBarth() == null ? school.getBarth() : form.getBarth());
        employee.setMarry(form.getMarry() == null ? school.getMarry() : form.getMarry());
        employee.setFuneral(form.getFuneral() == null ? school.getFuneral() : form.getFuneral());
        this.getRepository().save(employee);
    }
}
