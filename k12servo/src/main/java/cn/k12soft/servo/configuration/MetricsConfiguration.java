...
public class MetricsConfiguration extends MetricsConfigurerAdapter {
  ...
  public MetricsConfiguration(K12Properties k12Properties) {
...
//    metricRegistry.register(PROP_METRIC_REG_JCACHE_STATISTICS, new JCacheGaugeSet());
    ...
    if (k12Properties.getMetrics().getJmx().isEnabled()) {
      log.debug("Initializing Metrics JMX reporting");
      JmxReporter jmxReporter = JmxReporter.forRegistry(metricRegistry).build();
      jmxReporter.start();
    }
    if (k12Properties.getMetrics().getLogs().isEnabled()) {
      ...
      Slf4jReporter reporter = Slf4jReporter.forRegistry(metricRegistry)
        ...
        .build();
      reporter.start(k12Properties.getMetrics().getLogs().getReportFrequency(), TimeUnit.SECONDS);
    }
  }
}
